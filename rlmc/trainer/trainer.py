        ...
        for epoch in range(self.args.train.max_epochs):
            ...
            for batch_idx, (data, target) in enumerate(self.train_loader):
                ...
                if batch_idx % self.args.train.log_interval == 0:
                    ...
                    logger.info(
                        f"Train Epoch: {epoch} [{batch_idx}/{len(self.train_loader)}] "
                        + " iter_progress={:.2f}%  loss={:.6f}, acc={:.2f}%, macc={:.2f}%, mIoU={:.2f}%, fwIoU={:.2f}%".format(
            ...
            if self.args.train.is_eval and epoch != 0:
                if (
                    epoch % self.args.train.eval_interval == 0
    ...
    def evaluate(self, epoch=10000000, is_training=False):
        ...
        if is_training:
            ...
            logger.info(
                ...
                + " iter_progress={:.2f}%  loss={:.6f}, acc={:.2f}%, macc={:.2f}%, mIoU={:.2f}%, fwIoU={:.2f}%".format(
                    100 * val_batch_idx / len(self.val_loader),
                    val_loss.item(),
                    acc * 100,
                    macc * 100,
                    mIoU * 100,
            ...
            for k, v in state_dict.items():
                if (
                    "total_" in k
                ):  # 将torchsummary或thop生成的含total_的Unexpected key过滤掉，避免eval或predict设置为strict=True时读取模型state_dict报错
                    continue
                keys.append(k)
